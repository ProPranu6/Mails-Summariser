...
class SentenceEncoders():
  ...
  def text_to_sens(transcript_df=None, text=None, text_col='transcript', combine_every=1, minimum_sen_len = 2):
    ...
    if transcript_df == None:
        text = re.sub('\\n+|\s\s+'," ", text)
        text = re.sub('\*+','',text)
        #sens = re.split("(?<!\w\.\w.)(?<![A-Z][a-z]\.)(?<=\.|\?|\!)\s", text)
    else:
        text = "\n".join(transcript_df[text_col])
    """      
    sens_new = []
    count = 1
    lasting_sen = ""
    ...
    entire = summarizer(parser.document,int(1e+10)) 
    sens = []
    for sen in entire:
      sens.append(str(sen))
    return sens
...
class SummarisationAlgos():
    ...
    def __call__(self, sens_text=None, txt=None, file_name=None, sentence_count=None):
      if self.algo_name == SummarisationAlgos.list_algos[0]:
        ...
        return self.IDFLexRank(txt, file_name)
      elif self.algo_name == SummarisationAlgos.list_algos[2]:
        
        return self.AltAlgos(TEXT_FILE=file_name, LANGUAGE="english", SENTENCE_COUNT=sentence_count)
    ...
    class IDFLexRank():
      def __init__(self, txt=None, file_name=None):
        self.txt=txt
      ...
      def idf_vecs_lexrank(self, sentence_count=10):
        if self.txt == None:
          parser = PlaintextParser.from_file(self.file_name, Tokenizer("english"))
        else:
...
def produce_ensembled_summary(summ_dict, sentence_count=1, original_sens=None):
  ...
  for algo in summ_dict:
    ...
    #sens = re.split("(?<!\w\.\w.)(?<![A-Z][a-z]\.)(?<=\.|\?|\!)\s",summ_dict[algo])
    #sens = [ele.strip() for ele in sens if ele != "" and ele != " "]
    pool_summary += summ_dict[algo]
...
def execute_summarisation(txt="Provide the text here", summary_ratio=0.4):
    ...
    file_suffix = "__" + str(int(time.time()/1e+2))  
    path_to_original = "./Elite.ai-content-summarisations/input_text"+file_suffix+".txt"
    clean_txt = re.sub('(https:.*\\n)|([0-9]+:[0-9]+)', '', txt)
    

    Senc = SentenceEncoders(model_name='paraphrase-mpnet-base-v2')
    enc = Senc.encoder
    input_sens = SentenceEncoders.text_to_sens(text=clean_txt, combine_every=1)
    

   
    new_txt = "\n".join(input_sens)
    input_fl = open(path_to_original, "w")
    input_fl.write(new_txt)
    input_fl.close()

    Senc = SentenceEncoders(model_name='paraphrase-mpnet-base-v2')
    enc = Senc.encoder
    original_sens = SentenceEncoders.text_to_sens(text=new_txt, combine_every=1)
    ...
    algon = SummarisationAlgos('AltAlgos')(file_name=path_to_original, sentence_count=sentence_count)
    ...
    algo2 = SummarisationAlgos('IDFLexRank')(txt=new_txt)
    ...
    path_to_summary = "./Elite.ai-content-summarisations/output_summary"+file_suffix+".txt"
    ...
    print("~DONE~")
    return summ_dict
#sd = execute_summarisation(txt, summary_ratio=0.3)
